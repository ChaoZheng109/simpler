...
/**
 * Scalar Addition Kernel — UNINSTRUMENTED (no mark_stamp).
 *
 * Implements: out[i] = src[i] + scalar
 *
 * In the vector_mul_stamp example only kernel_mul carries mark_stamp
 * instrumentation, so every biu_perf pipe stamp the run produces is
 * attributable to the single kernel_mul task. This kernel is deliberately
 * left clean so it contributes no mark_stamp events.
 */
