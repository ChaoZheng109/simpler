...
static pthread_key_t g_aicore_l0_perf_ring_key;
...
static void create_tls_keys() {
    ...
    pthread_key_create(&g_aicore_l0_perf_ring_key, nullptr);
...
__aicore__ void set_aicore_l0_perf_ring(__gm__ L0PerfAicoreRing *ring) {
    pthread_setspecific(g_aicore_l0_perf_ring_key, reinterpret_cast<void *>(ring));
}
__aicore__ __gm__ L0PerfAicoreRing *get_aicore_l0_perf_ring() {
    return reinterpret_cast<__gm__ L0PerfAicoreRing *>(pthread_getspecific(g_aicore_l0_perf_ring_key));
}
...
extern "C" void aicore_execute_wrapper(
    ...
    if ((enable_profiling_flag & PROFILING_FLAG_L0_SWIMLANE) && aicore_l0_perf_ring_addrs != 0) {
        uint64_t *l0_ring_table = reinterpret_cast<uint64_t *>(aicore_l0_perf_ring_addrs);
