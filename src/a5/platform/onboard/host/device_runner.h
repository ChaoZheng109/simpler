 ...
 * specific offsets for aicpu_so_bin and aicpu_so_len fields. The fields are
 * load-bearing on a5 onboard (CI surfaced 207001 AICore launch failures and
 * 507899 stream-create failures when they were dropped); treat the layout as
 * part of the device-side contract even though our own kernels do not read it.
 */
struct DeviceArgs {
 ...
 *
 * Manages the host→device copy of the runtime AICPU SO bytes that backs
 * DeviceArgs.aicpu_so_bin / aicpu_so_len. Required on a5 onboard even though
 * our own runtime AICPU SO never dereferences these fields — removing the
 * H2D allocation destabilized CI (see DeviceArgs comment above).
 */
struct AicpuSoInfo {
    uint64_t aicpu_so_bin{0};
    uint64_t aicpu_so_len{0};
...
public:
    ...
    void set_l0_swimlane_enabled(bool enable) { enable_l0_swimlane_ = enable; }
...
private:
    ...
    // simpler_init. aicore_kernel_binary_ stays resident (launch_aicore_kernel
    // re-registers it via rtRegisterAllKernel on every run). aicpu_so_binary_
    // is released by ensure_binaries_loaded() after bootstrap; bootstrap is
    // the only consumer and Mode B per-task launches go through the cached
    // rtFuncHandle on LoadAicpuOp, not the host bytes.
    ...
    // L0 core swimlane (per-pipe biu_perf HW stamps)
    L0PerfCollector l0_perf_collector_;
    ...
    bool enable_l0_swimlane_{false};
