...
int run_prepared(
    ...
    // L0 swimlane (biu_perf) is dav-c310 (a5) hardware; a2a3 SoCs do not have
    // this HW path. Fail-fast so users see the unsupported flag immediately
    // rather than silently getting no l0_perf_records.json.
    if (enable_l0_swimlane != 0) {
        LOG_ERROR(
            "run_prepared: --enable-l0-swimlane is not supported on a2a3 "
