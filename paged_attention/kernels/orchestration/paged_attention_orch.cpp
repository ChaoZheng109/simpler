...
int build_paged_attention_graph(Runtime* runtime, uint64_t* args, int arg_count) {
    ...
    for (int b_idx = 0; b_idx < batch; b_idx++) {
        ...
        for (int bn = 0; bn < bn_this_batch; bn++) {
            ...
            int t_qk = runtime->add_task(qk_args, 7, FUNC_QK_MATMUL, CoreType::AIC, "QK");
            total_tasks++;
            ...
            int t_sf = runtime->add_task(sf_args, 8, FUNC_SOFTMAX_PREPARE, CoreType::AIV, "SF");
            total_tasks++;
            ...
            int t_pv = runtime->add_task(pv_args, 7, FUNC_PV_MATMUL, CoreType::AIC, "PV");
            total_tasks++;
        ...
        for (int bn = 0; bn < bn_this_batch; bn++) {
            ...
            int t_up = runtime->add_task(up_args, 11, FUNC_ONLINE_UPDATE, CoreType::AIV, "UP");
            total_tasks++;
