...
Use  to run the simulation and generate a Mermaid task dependency graph:

```bash
# Generate full graph (all batches)
python3 gen_mermaid_graph.py

# Generate graph for batch 0 only
python3 gen_mermaid_graph.py --batch 0

# Custom output file
python3 gen_mermaid_graph.py -o my_graph.md
```

The generated Mermaid graph shows task dependencies with AIC kernels in blue and AIV kernels in orange.

## Generate Task Graph

Use `gen_mermaid_graph.py` to run the simulation and generate a Mermaid task dependency graph:
